(function ($) {
	...
	CherryJsCore.theme_script = {
		...
		smart_slider_init: function (self) {
			$('.techloop-smartslider').each(function () {
				var slider = $(this),
		...
		swiper_carousel_init: function (self) {
			...
			jQuery('.techloop-carousel').each(function () {
				var swiper = null,
		...
		navbar_init: function (self) {
			...
			$(window).load(function () {
				...
				var $layout = techloop.labels.header_layout,
		...
		subscribe_init: function (self) {
			CherryJsCore.variable.$document.on('click', '.subscribe-block__submit', function (event) {
				...
				$.ajax({
					url: techloop.ajaxurl,
					type: 'post',
					dataType: 'json',
					data: {
						action: 'techloop_subscribe',
						mail: mail,
						nonce: nonce
					},
					error: function () {
		...
		main_menu: function (self, $mainNavigation) {
			...
			if (hasprop.call(window, 'techloop') &&
				hasprop.call(window.techloop, 'more_button_options') &&
				hasprop.call(window.techloop.more_button_options, 'more_button_type')) {
				switch (window.techloop.more_button_options.more_button_type) {
					case 'image':
						imgurl = window.techloop.more_button_options.more_button_image_url;
						if (window.techloop.more_button_options.retina_more_button_image_url) {
							srcset = ' srcset="' + window.techloop.more_button_options.retina_more_button_image_url + ' 2x"';
						}
						moreMenuContent = '<img src="' + imgurl + '"' + srcset + ' alt="' + moreMenuContent + '">';
						hasimg = true;
						break;
					case 'icon':
						moreMenuContent = '<i class="fa ' + window.techloop.more_button_options.more_button_icon + '"></i>';
						hasicon = true;
						break;
					case 'text':
					default:
						moreMenuContent = window.techloop.more_button_options.more_button_text || moreMenuContent;
						hasimg = false;
						hasicon = false;
						break;
				}
		...
		to_top_init: function (self) {
			if ($.isFunction(jQuery.fn.UItoTop)) {
				$().UItoTop({
					text: techloop.labels.totop_button,
					scrollSpeed: 600
				});
			}
		},
	...
	window.onresize = function () {
		...
		if (windWidthRes < 992 && headerDefault && verticalMenuWrapBlock) {
	...
	if (windWidth < 992 && headerDefault && verticalMenuWrapBlock) {
		headerDefault.appendChild(verticalInner);
	}
	if (topPanelMenuListUL) {
