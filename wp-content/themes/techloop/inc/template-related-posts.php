 ...
 * @return array
 */
function techloop_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = get_theme_mod( 'related_posts_visible', techloop_theme()->customizer->get_default( 'related_posts_visible' ) );
	...
	$post_terms  = '';
	$post_number = get_theme_mod( 'related_posts_count', techloop_theme()->customizer->get_default( 'related_posts_count' ) );
	...
	$holder_view_dir = locate_template( apply_filters( 'techloop_related_posts_template_part_slug', 'template-parts/content-related-posts.php' ), false, false );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = get_theme_mod( $setting_value, techloop_theme()->customizer->get_default( $setting_value ) );
	}

	$settings['title_visible']   = $settings['title_length'] ? get_theme_mod( 'related_posts_title', techloop_theme()->customizer->get_default( 'related_posts_title' ) ) : false;
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$comment_count = $utility->meta_data->get_comment_count( array(
				...
				'sufix'   => get_comments_number_text( esc_html__( 'No comment(s)', 'techloop' ), esc_html__( '1 comment', 'techloop' ), esc_html__( '% comments', 'techloop' ) ),
			) );
