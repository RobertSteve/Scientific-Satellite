...
if ( ! class_exists( 'Techloop_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class Techloop_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'techloop' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'techloop' );
			$this->widget_id          = 'techloop_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => TECHLOOP_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'techloop' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'techloop' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'techloop' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'techloop' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'techloop' ),
						'categories' => esc_html__( 'Show categories', 'techloop' ),
						'tags'       => esc_html__( 'Show tags', 'techloop' ),
						'author'     => esc_html__( 'Show author', 'techloop' ),
						'date'       => esc_html__( 'Show date', 'techloop' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'techloop' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'techloop' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template(  apply_filters( 'techloop_featured_posts_block_widget_view_dir', 'inc/widgets/featured-posts-block/views/item.php' ), false, false );
			 ...
			 */
			$query = apply_filters( 'techloop_featured_posts_block_query', $query, $this->instance );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'techloop' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'techloop' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'techloop' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'techloop' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'techloop' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'techloop_register_featured_posts_block_widget' );

	if ( false === function_exists( 'techloop_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function techloop_register_featured_posts_block_widget() {
			register_widget( 'Techloop_Featured_Posts_Block_Widget' );
		}
	}
}
