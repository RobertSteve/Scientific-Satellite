...
if ( ! class_exists( 'Techloop_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Techloop_Playlist_Slider_Widget.
	 */
	class Techloop_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'techloop' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'techloop' );
			$this->widget_id          = apply_filters( 'techloop_playlist_slider_widget_ID', 'techloop_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'techloop_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = techloop_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'techloop' ),
				...
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'techloop' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'techloop' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'techloop' ),
						true  => esc_html__( 'Buttons', 'techloop' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'techloop' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'techloop' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'techloop' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'techloop' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
				...
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'techloop' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						...
						'comments' => esc_html__( 'Comment count', 'techloop' ),
				...
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'techloop' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'techloop_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'techloop-thumb-s',
					'mobile_size' => 'techloop-thumb-s',
				...
				$comments = $this->utility->meta_data->get_comment_count( array(
					...
					'html'    => '<span class="post__comments">%1$s%5$s <span class="text">%6$s</span></span>',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'techloop_playlist_buttons', false ),
...
add_action( 'widgets_init', 'techloop_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function techloop_register_playlist_slider_widget() {
	register_widget( 'Techloop_Playlist_Slider_Widget' );
}
