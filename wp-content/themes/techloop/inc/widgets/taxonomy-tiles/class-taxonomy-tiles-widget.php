...
if ( ! class_exists( 'Techloop_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Techloop_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'techloop' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'techloop' );
			$this->widget_id          = 'techloop-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				'title' => array(
					...
					'label' => esc_html__( 'Widget title', 'techloop' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category',
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'techloop' ),
				...
				'post_tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'techloop' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'techloop' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'techloop' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'techloop' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'techloop' ),
						),
					),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'techloop' ),
				...
				'items_padding' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'techloop' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'techloop_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'techloop_taxonomy_tiles_widget_before',
					'<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '">',
					$instance
				);

				techloop_theme()->dynamic_css->add_style(
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s Post', '%s Posts', 'techloop' ),
				...
				echo apply_filters( 'techloop_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'techloop_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function techloop_register_taxonomy_tiles_widget() {
		register_widget( 'Techloop_Taxonomy_Tiles_Widget' );
	}
}
