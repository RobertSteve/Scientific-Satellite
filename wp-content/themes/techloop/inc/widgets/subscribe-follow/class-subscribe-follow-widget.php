...
class Techloop_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'techloop' );
	$this->widget_id          = 'techloop_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'techloop' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'techloop' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'techloop' ),
				'label' => esc_html__( 'Subscribe Title', 'techloop' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'techloop' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'techloop' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'techloop' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'techloop' ),
				'label' => esc_html__( 'Subscribe submit label', 'techloop' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'techloop' ),
				'label' => esc_html__( 'Subscribe success', 'techloop' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'techloop' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'techloop' ),
				'label' => esc_html__( 'Follow Title', 'techloop' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'techloop' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'techloop' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'techloop' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'techloop' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'techloop' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'techloop' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'techloop' ),
					'top-center'    => esc_html__( 'Top Center', 'techloop' ),
					'top-right'     => esc_html__( 'Top Right', 'techloop' ),
					'center-left'   => esc_html__( 'Middle Left', 'techloop' ),
					'center'        => esc_html__( 'Middle Center', 'techloop' ),
					'center-right'  => esc_html__( 'Middle Right', 'techloop' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'techloop' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'techloop' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'techloop' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'techloop' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'techloop' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'techloop' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'techloop' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'techloop' ),
					'contain' => esc_html__( 'Contain', 'techloop' ),
					'auto'    => esc_html__( 'Auto', 'techloop' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'techloop' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_techloop_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_techloop_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'techloop' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'techloop' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'techloop' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'techloop' ),
		);

		$this->nonce = wp_create_nonce( 'techloop-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			echo '<span class="not-api-key">' . esc_html__( 'Please set up MailChimp API key and List ID', 'techloop' ) . '</span>';
		}
	...
	public function get_social_nav() {
		return techloop_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = techloop_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'techloop-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'techloop_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function techloop_register_subscribe_follow_widgets() {
	register_widget( 'Techloop_Subscribe_Follow_Widget' );
}
