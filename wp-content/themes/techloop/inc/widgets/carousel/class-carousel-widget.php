 ...
 * @package Techloop
 */
if ( ! class_exists( 'Techloop_Carousel_Widget' ) ) {

	/**
	 * Class Techloop_Carousel_Widget.
	 */
	class Techloop_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'techloop' );
			$this->widget_id          = 'techloop_widget_carousel';
			$this->widget_name        = esc_html__( 'Carousel', 'techloop' );
			...
			$this->settings           = array(
				...
				'trim_title' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Title char trimmed count', 'techloop' ),
				),
				'trim_words' => array(
					'type'       => 'slider',
					'value'      => 15,
				...
				'slides_per_view' => array(
					...
					'min_value' => 1,
					'value'     => 3,
					'label'     => esc_html__( 'Number of slides per view', 'techloop' ),
				),
				'slides_per_group' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Number slides per group', 'techloop' ),
				),
				'slides_per_column' => array(
					...
					'min_value' => 1,
					'value'     => 1,
					'label'     => esc_html__( 'Multi Row Slides Layout', 'techloop' ),
				),
				'space_between_slides' => array(
					...
					'min_value' => 0,
					'value'     => 30,
					'label'     => esc_html__( 'Width of the space between slides(px)', 'techloop' ),
				),
				'duration_speed' => array(
					...
					'min_value' => 100,
					'value'     => 500,
					'label'     => esc_html__( 'Duration of transition between slides (ms)', 'techloop' ),
				),
				'navigation' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider navigation', 'techloop' ),
				),
				'pagination' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Slider pagination', 'techloop' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'techloop_carousel_widget_view_dir', 'inc/widgets/carousel/views/carousel-view.php' ), false, false );
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', techloop_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="techloop-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'techloop' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'techloop_carousel_default_query_args', array(
				'post_type'      => 'post',
				'orderby'        => 'date',
				'order'          => 'DESC',
				'posts_per_page' => - 1,
		...
		public function get_carousel_loop( $posts_query, $template ) {
			...
			if ( ! $template  ) {
				return '<h5>' . esc_html__( 'View file not found', 'techloop' ) . '</h5>';
			}
	...
	add_action( 'widgets_init', 'techloop_register_carousel_widget' );
	/**
	 * Register carousel widget.
	 */
	function techloop_register_carousel_widget() {
		register_widget( 'Techloop_Carousel_Widget' );
	}
}
