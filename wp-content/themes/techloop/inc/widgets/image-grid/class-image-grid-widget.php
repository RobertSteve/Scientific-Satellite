 ...
 * @package Techloop
 */
if ( ! class_exists( 'Techloop_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Techloop_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'techloop' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'techloop' );
			...
			$this->settings           = array(
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'techloop' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'techloop' ),
						'title'         => esc_html__( 'Post Title', 'techloop' ),
						'comment_count' => esc_html__( 'Comment Count', 'techloop' ),
					),
					'label'   => esc_html__( 'Post sorted', 'techloop' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'techloop' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'techloop' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'techloop_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'techloop_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'techloop_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'techloop_image_grid_widget_before',
				...
				echo apply_filters( 'techloop_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'techloop_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function techloop_register_image_grid_widget() {
		register_widget( 'Techloop_Image_Grid_Widget' );
	}
}
