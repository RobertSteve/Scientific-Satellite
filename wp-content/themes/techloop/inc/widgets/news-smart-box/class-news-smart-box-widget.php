...
/**
 * Class Techloop_News_Smart_Box_Widget
 */
class Techloop_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'techloop_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'New Smart Box', 'techloop' );
		$this->widget_description = esc_html__( 'New Smart Box', 'techloop' );
		$this->utility            = techloop_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'techloop' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'techloop' ),
						'img_src' => TECHLOOP_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'techloop' ),
						'img_src' => TECHLOOP_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'techloop' ),
						'img_src' => TECHLOOP_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'techloop' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'techloop' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'techloop' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'techloop' ),
				'placeholder'      => esc_html__( 'Select tag', 'techloop' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'techloop' ),
				'multiple'         => true,
			...
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'techloop' ),
			),
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'techloop' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'techloop' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'techloop' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'techloop' ),
			),
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'techloop_news_smart_box_default_query_args', array(
			'posts_per_page' => '5',
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<h4>' . esc_html__( 'Posts not found', 'techloop' ) . '</h4>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$author = $this->utility->meta_data->get_author(array(
				...
				'html'    => '<span class="news-smart-box__item-author  posted-by">%1$s<a href="%2$s" class="posted-by__author" %3$s %4$s rel="author">%5$s%6$s</a></span>',
			) );
		...
		$html .= '<div class="news-smart-box__navigation">';
			...
			$html .= '<div class="news-smart-box__navigation-current-term">';
				if ( 'true' === $this->instance['current_title_visibility'] ) {
					$html .= '<h5 class="news-smart-box__navigation-title">' . $current_term_name . '</h5>';
				}
			...
			if ( ! empty( $alt_terms_slug_list ) ) {
				$html .= '<div class="news-smart-box__navigation-wrapper">';
					$html .= '<div class="news-smart-box__navigation-terms-list link">';
						foreach ( $alt_terms_slug_list as $key => $term_slug ) {
							$name = $this->get_term_name_by_slug( $term_slug );

							if ( $name ) {
								$html .= '<div class="news-smart-box__navigation-terms-list-item link__text" data-slug=' . $term_slug . '>' . $this->get_term_name_by_slug( $term_slug ) . '</div>';
							}
						}
					$html .= '</div>';
				$html .= '</div>';
...
add_action( 'widgets_init', 'techloop_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function techloop_register_news_smart_box_widgets() {
	register_widget( 'Techloop_News_Smart_Box_Widget' );
}
