...
if ( ! class_exists( 'Techloop_Smart_Slider_Widget' ) ) {

	/**
	 * Class Techloop_Smart_Slider_Widget.
	 */
	class Techloop_Smart_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display smart slider on your site.', 'techloop' );
			$this->widget_id          = 'techloop_widget_smart_slider';
			$this->widget_name        = esc_html__( 'Smart Slider', 'techloop' );
			...
			$this->settings           = array(
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 1,
					'label'     => esc_html__( 'Posts count', 'techloop' ),
				),
				'post_title' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display title', 'techloop' ),
				),
				'content' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display content', 'techloop' ),
				),
				'more_button' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display more button', 'techloop' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'techloop' ),
						'false_toggle' => esc_html__( 'Off', 'techloop' ),
				...
				'more_button_text' => array(
					...
					'label'  => esc_html__( 'More button text', 'techloop' ),
				...
				'trim_words' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Content words trimmed count', 'techloop' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width', 'techloop' ),
				),
				'height' => array(
					'type'  => 'text',
					'value' => '800',
					'label' => esc_html__( 'Slider height', 'techloop' ),
				),
				'orientation' => array(
					'type'    => 'select',
					'size'    => 1,
					'value'   => 'horizontal',
					'options' => array(
						'horizontal' => esc_html__( 'Horizontal', 'techloop' ),
						'vertical'   => esc_html__( 'Vertical', 'techloop' ),
					),
					'label'   => esc_html__( 'Slider orientation', 'techloop' ),
				),
				'slide_distance' => array(
					...
					'max_value' => 100,
					'min_value' => 0,
					'label'     => esc_html__( 'Slide distance(px)', 'techloop' ),
				),
				'slide_duration' => array(
					...
					'min_value'  => 100,
					'step_value' => 100,
					'label'      => esc_html__( 'Slide duration(ms)', 'techloop' ),
				),
				'slide_fade' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use fade effect?', 'techloop' ),
				),
				'navigation' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Use navigation?', 'techloop' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'techloop' ),
						'false_toggle' => esc_html__( 'Off', 'techloop' ),
				...
				'fade_navigation' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Indicates whether the arrows will fade in only on hover', 'techloop' ),
				...
				'pagination' => array(
					...
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use pagination?', 'techloop' ),
				),
				'autoplay' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use autoplay?', 'techloop' ),
				),
				'fullScreen' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display fullScreen button?', 'techloop' ),
				),
				'shuffle' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Indicates if the slides will be shuffled', 'techloop' ),
				),
				'loop' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use infinite scrolling?', 'techloop' ),
				),
				'thumbnails' => array(
					'type'   => 'switcher',
					'value'  => 'false',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails?', 'techloop' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'techloop' ),
						'false_toggle' => esc_html__( 'Off', 'techloop' ),
				...
				'thumbnails_position' => array(
					...
					'options' => array(
						...
						'bottom' => esc_html__( 'Bottom', 'techloop' ),
						'right'  => esc_html__( 'Right', 'techloop' ),
						'left'   => esc_html__( 'Left', 'techloop' ),
					),
					'label'   => esc_html__( 'Sets the position of the thumbnail scroller', 'techloop' ),
				...
				'thumbnails_arrows' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails arrows?', 'techloop' ),
					'master' => 'thumbnails_attr',
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( $posts_query ) {
				...
				$html = '<h4>' . esc_html__( 'Posts for slider not found', 'techloop' ) . '</h4>';
			}
		...
		public function render_slider( $posts_query ) {
			...
			$html = '<div class="techloop-smartslider smart-slider__instance invert" ' . $slider_html_attr . '>';
		...
		public function get_query_slider_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'techloop_smart_slider_default_query_args', array(
				'post_type'           => 'post',
				'orderby'             => 'date',
				'order'               => 'DESC',
				'posts_per_page'      => - 1,
		...
		public function get_slider_loop( $posts_query ) {
			 ...
			 * @var  array
			 */
			$title_settings = apply_filters( 'techloop_smart_slider_title_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '38%%',
			 ...
			 * @var  array
			 */
			$content_settings = apply_filters( 'techloop_smart_slider_content_settings', array(
				'width'           => '100%%',
				'horizontal'      => '5%%',
				'vertical'        => '51%%',
				'show_transition' => 'left',
			 ...
			 * @var  array
			 */
			$more_settings = apply_filters( 'techloop_smart_slider_more_button_settings', array(
				'width'           => '100%%',
				'horizontal'      => '5%%',
				'vertical'        => '58%%',
				'show_transition' => 'left',
		...
		public function get_slider_thumbnails( $posts_query ) {
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				$html .= '<div class="sp-thumbnail">';
					$html .= $this->utility->media->get_image( array(
						'size'        => 'techloop-slider-thumb',
						'mobile_size' => 'techloop-slider-thumb',
	...
	add_action( 'widgets_init', 'techloop_register_smart_slider_widgets' );
	/**
	 * Register Smart slider widget.
	 */
	function techloop_register_smart_slider_widgets() {
		register_widget( 'Techloop_Smart_Slider_Widget' );
	}
}
