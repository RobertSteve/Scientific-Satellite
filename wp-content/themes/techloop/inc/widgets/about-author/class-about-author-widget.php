 ...
 * @package Techloop
 */
if ( ! class_exists( 'Techloop_About_Author_Widget' ) ) {

	/**
	 * Class Techloop_About_Author_Widget.
	 */
	class Techloop_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'techloop widget-about-author invert';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'techloop' );
			$this->widget_id          = 'techloop_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'techloop' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'techloop' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'techloop' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 190,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'techloop' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'techloop' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'techloop' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'techloop' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'techloop' ),
					'label' => esc_html__( 'Link label', 'techloop' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'techloop' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'techloop-author-avatar' );
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'techloop_widget_about_author_view', 'inc/widgets/about-author/view/about-author.php' ), false, false );
...
add_action( 'widgets_init', 'techloop_register_about_author_widgets' );
/**
 * Register about-author widget.
 */
function techloop_register_about_author_widgets() {
	register_widget( 'Techloop_About_Author_Widget' );
}
