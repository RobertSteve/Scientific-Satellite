...
if ( ! class_exists( 'Techloop_About_Widget' ) ) {

	/**
	 * Class Techloop_About_Widget.
	 */
	class Techloop_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'techloop' );
			$this->widget_id          = 'techloop_widget_about';
			$this->widget_name        = esc_html__( 'About Techloop', 'techloop' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'techloop' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'techloop' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'techloop' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'techloop' ),
					'value'             => '',
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, techloop_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'techloop_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'techloop_register_about_widget' );
/**
 * Register about widget.
 */
function techloop_register_about_widget() {
	register_widget( 'Techloop_About_Widget' );
}
