 ...
 * @return array
 */
function techloop_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! techloop_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'techloop_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'techloop_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'techloop' ) . $network['name'],
	...
	if ( 'loop' == $context ) {
		$format = '<div class="share-btns__list-wrap">
						<div class="linearicon linearicon-share2 icon-toggle-share-btns"></div>
						<div class="share-btns__list %1$s">%2$s</div>
					</div>';
	} else {
		$format = '<div class="share-btns__list %1$s">%2$s</div>';
	}

	$format = apply_filters( 'techloop_share_buttons_html_format', $format, $context );

	printf( $format, esc_attr( $config[ 'custom_class' ] ), $share_buttons );
}
 ...
 * @return string
 */
function techloop_get_single_share_buttons() {
	ob_start();
	techloop_share_buttons( 'single' );
