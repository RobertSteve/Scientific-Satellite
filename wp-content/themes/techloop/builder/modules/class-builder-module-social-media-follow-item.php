...
class Tm_Builder_Module_Social_Media_Follow_Item extends Tm_Builder_Module {
	public function init() {
		$this->name                        = esc_html__( 'Social Network', 'techloop' );
		...
		$this->fields_defaults = array(
			'url'            => array( '#' ),
			'bg_color'       => array( apply_filters( 'tm_builder_accent_color', tm_get_option( '', '' )), 'only_default_setting' ),
			'hover_bg_color' => array( apply_filters( 'tm_builder_accent_color', tm_get_option( '', '' )), 'only_default_setting' ),
			'skype_action'   => array( 'call' ),
		);

		$this->advanced_setting_title_text = esc_html__( 'New Social Network', 'techloop' );
		$this->settings_text               = esc_html__( 'Social Network Settings', 'techloop' );

		$this->custom_css_options = array(
			'social_icon' => array(
				'label'    => esc_html__( 'Social Icon', 'techloop' ),
				'selector' => 'a.icon',
			),
			'follow_button' => array(
				'label'    => esc_html__( 'Follow Button', 'techloop' ),
	...
	public function get_fields() {
		$fields = array(
			'social_network' => array(
				'label'           => esc_html__( 'Social Network Label', 'techloop' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'class'           => 'tm-pb-social-network',
				'description'     => esc_html__( 'Set the social network label', 'techloop' ),
			),
			'social_icon' => array(
				...
				'description' => esc_html__( 'Choose the social network icon', 'techloop' ),
			),
			'content_new' => array(
			...
			'url' => array(
				'label'               => esc_html__( 'Account URL', 'techloop' ),
				'type'                => 'text',
				'option_category'     => 'basic_option',
				'description'         => esc_html__( 'The URL for this social network link.', 'techloop' ),
				'depends_show_if_not' => 'f17e',
			),
			'skype_url' => array(
				'label'           => esc_html__( 'Account Name', 'techloop' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'The Skype account name.', 'techloop' ),
				'depends_show_if' => 'f17e',
			),
			'skype_action' => array(
				'label'           => esc_html__( 'Skype Button Action', 'techloop' ),
				'type'            => 'select',
				'option_category' => 'basic_option',
				'options'         => array(
					'call' => esc_html__( 'Call', 'techloop' ),
					'chat' => esc_html__( 'Chat', 'techloop' ),
				),
				'depends_show_if' => 'f17e',
				'description'     => esc_html__( 'Here you can choose which action to execute on button click', 'techloop' ),
			),
			'color' => array(
				'label'           => esc_html__( 'Icon Color', 'techloop' ),
				'type'            => 'color-alpha',
				'description'     => esc_html__( 'This will change the icon color.', 'techloop' ),
				'additional_code' => '<span class="tm-pb-reset-setting reset-default-color" style="display: none;"></span>',
			),
			'bg_color' => array(
				'label'           => esc_html__( 'Icon Background Color', 'techloop' ),
				'type'            => 'color-alpha',
				'description'     => esc_html__( 'This will change the icon background color.', 'techloop' ),
				'additional_code' => '<span class="tm-pb-reset-setting reset-default-color" style="display: none;"></span>',
			),
			'hover_icon_color' => array(
				'label'           => esc_html__( 'Icon Hover Color', 'techloop' ),
				'type'            => 'color-alpha',
				'description'     => esc_html__( 'This will change the icon hover background color.', 'techloop' ),
				'additional_code' => '<span class="tm-pb-reset-setting reset-default-color" style="display: none;"></span>',
			),
			'hover_bg_color' => array(
				'label'           => esc_html__( 'Hover Background Color', 'techloop' ),
				'type'            => 'color-alpha',
				'description'     => esc_html__( 'This will change hover background color.', 'techloop' ),
				'additional_code' => '<span class="tm-pb-reset-setting reset-default-color" style="display: none;"></span>',
			),
		);
		return $fields;
