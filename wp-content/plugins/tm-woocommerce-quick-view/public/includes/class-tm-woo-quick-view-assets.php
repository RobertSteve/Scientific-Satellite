...
if ( ! class_exists( 'TM_Woo_Quick_View_Assets' ) ) {
	...
	class TM_Woo_Quick_View_Assets {
		...
		/**
		 * Prepare additional quick view scipts and styles.
		 *
		 * @return
		 */
		public function get_quick_view_assets() {

			do_action( 'wp_enqueue_scripts' );

			$scripts = apply_filters( 'tm_woo_quick_view_add_scripts', array(
				'underscore',
				'wp-util',
				'wc-single-product',
				'wc-add-to-cart',
			...
			$styles = apply_filters( 'tm_woo_quick_view_add_styles', array() );

			$prepared_scripts = array();
			$prepared_styles  = array();
			...
			if ( ! empty( $styles ) ) {
				...
				foreach ( $styles as $handle ) {
					...
					$prepared_styles[ $handle ] = ob_get_clean();
				}
			}

			$vars = array();

			if ( in_array( 'wc-add-to-cart-variation', $scripts ) ) {
				ob_start();
			...
			return array(
				'scripts' => $prepared_scripts,
				'styles'  => $prepared_styles,
				'vars'    => $vars,
			);
		}
